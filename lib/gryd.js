      ...
      loadModules(result, "Mod", function (result) {
        ...
        loadModules(result, "Srv", function (result) {
          ...
          loadModules(result, "Ctrl", function (result) {
            ...
            Gryd.App.listen(Gryd.Settings.port, function() {
              if (Gryd.Settings.callbackFn) {
                Gryd.Settings.callbackFn();
              }
            });
...
function loadDB (env, callback) {
  if (!(env && env.Config && env.Config.db)) {
    callback(env);
    return;
  }

  Mongoose.connect(env.Config.db, { maxPoolSize: 300 })
    .then(db => {
      env.DB = db;
      callback(env);  
    })
        ...
        fs.readdir(path, function (err, files) {
          if (err) {
            ...
            for (var i in files) {
              ...
              // prevent gryd to load test files
              // on running server node server crashing
              // missing jest functions
